...
#if ! (                                                                                                                \
    defined(SOKOL_GLCORE) || defined(SOKOL_GLES3) || defined(SOKOL_D3D11) || defined(SOKOL_METAL) ||                   \
    defined(SOKOL_WGPU) || defined(SOKOL_DUMMY_BACKEND))
#error                                                                                                                 \
...
_SOKOL_PRIVATE void
_sg_attachments_common_init(_sg_attachments_common_t* cmn, const sg_attachments_desc* desc, int width, int height)
{
...
#define _SG_WGPU_MAX_BINDGROUP_ENTRIES                                                                                 \
    (SG_NUM_SHADER_STAGES *                                                                                            \
     (SG_MAX_SHADERSTAGE_IMAGES + SG_MAX_SHADERSTAGE_SAMPLERS + SG_MAX_SHADERSTAGE_STORAGEBUFFERS))
...
_SOKOL_PRIVATE bool _sg_strempty(const _sg_str_t* str) { return 0 == str->buf[0]; }

_SOKOL_PRIVATE const char* _sg_strptr(const _sg_str_t* str) { return &str->buf[0]; }
...
_SOKOL_PRIVATE bool _sg_is_depth_stencil_format(sg_pixel_format fmt) { return (SG_PIXELFORMAT_DEPTH_STENCIL == fmt); }
...
_SOKOL_PRIVATE int _sg_roundup(int val, int round_to) { return (val + (round_to - 1)) & ~(round_to - 1); }
...
_SOKOL_PRIVATE bool _sg_multiple_u64(uint64_t val, uint64_t of) { return (val & (of - 1)) == 0; }
...
_SOKOL_PRIVATE int _sg_miplevel_dim(int base_dim, int mip_level) { return _sg_max(base_dim >> mip_level, 1); }
...
_SOKOL_PRIVATE void _sg_pixelformat_s(_sg_pixelformat_info_t* pfi) { pfi->sample = true; }
...
_sg_dummy_create_pipeline(_sg_pipeline_t* pip, _sg_shader_t* shd, const sg_pipeline_desc* desc)
{
...
_SOKOL_PRIVATE sg_resource_state _sg_dummy_create_attachments(
...
_SOKOL_PRIVATE void _sg_gl_load_opengl(void)
{
    ...
    _sg_wglGetProcAddress wgl_getprocaddress =
        (_sg_wglGetProcAddress)GetProcAddress(_sg.gl.opengl32_dll, "wglGetProcAddress");
...
_SOKOL_PRIVATE void
_sg_gl_init_pixelformats_float(bool has_colorbuffer_float, bool has_texture_float_linear, bool has_float_blend)
{
...
{
    SOKOL_ASSERT(
        (GL_ARRAY_BUFFER == target) || (GL_ELEMENT_ARRAY_BUFFER == target) || (GL_SHADER_STORAGE_BUFFER == target));
...
{
    _SG_GL_CHECK_ERROR();
    for (int i = 0; (i < _SG_GL_TEXTURE_SAMPLER_CACHE_SIZE) && (i < _sg.limits.gl_max_combined_texture_image_units);
         i++)
    {
...
_SOKOL_PRIVATE sg_resource_state
_sg_gl_create_pipeline(_sg_pipeline_t* pip, _sg_shader_t* shd, const sg_pipeline_desc* desc)
{
...
_SOKOL_PRIVATE void
_sg_gl_fb_attach_texture(const _sg_gl_attachment_t* gl_att, const _sg_attachment_common_t* cmn_att, GLenum gl_att_type)
{
...
_SOKOL_PRIVATE sg_resource_state _sg_gl_create_attachments(
    _sg_attachments_t*         atts,
    _sg_image_t**              color_images,
    _sg_image_t**              resolve_images,
    ...
    if ((_sg.gl.cache.cur_pipeline != pip) || (_sg.gl.cache.cur_pipeline_id.id != pip->slot.id))
    {
        ...
        // update stencil state
        {
            ...
            for (int i = 0; i < 2; i++)
            {
                ...
                if ((state_sfs->compare != cache_sfs->compare) || (state_ss->read_mask != cache_ss->read_mask) ||
        ...
        if (pip->cmn.color_count > 0)
        {
                ...
                (state_bs->dst_factor_alpha != cache_bs->dst_factor_alpha))
            {
                ...
                glBlendFuncSeparate(
                    _sg_gl_blend_factor(state_bs->src_factor_rgb),
...
_SOKOL_PRIVATE bool _sg_gl_apply_bindings(_sg_bindings_t* bnd)
{
    ...
    for (int stage_index = 0; stage_index < SG_NUM_SHADER_STAGES; stage_index++)
    {
        ...
        _SOKOL_UNUSED(num_imgs);
        SOKOL_ASSERT(num_smps == stage->num_samplers);
        _SOKOL_UNUSED(num_smps);
        for (int img_smp_index = 0; img_smp_index < stage->num_image_samplers; img_smp_index++)
        {
    ...
    for (GLuint attr_index = 0; attr_index < (GLuint)_sg.limits.max_vertex_attrs; attr_index++)
    {
        ...
        if (attr->vb_index >= 0)
        {
            ...
            if ((gl_vb != cache_attr->gl_vbuf) || (attr->size != cache_attr->gl_attr.size) ||
                (attr->type != cache_attr->gl_attr.type) || (attr->normalized != cache_attr->gl_attr.normalized) ||
                (attr->stride != cache_attr->gl_attr.stride) || (vb_offset != cache_attr->gl_attr.offset) ||
...
#define _sg_d3d11_setlabel(self, label)                                                                                \
    _sg_d3d11_SetPrivateData(                                                                                          \
        self,                                                                                                          \
        _sg_win32_refguid(_sg_d3d11_WKPDID_D3DDebugObjectName),                                                        \
        label ? (UINT)strlen(label) : 0,                                                                               \
        label)
#else
#define _sg_d3d11_setlabel(self, label)
#endif
...
static inline void _sg_d3d11_OMSetBlendState(
    ID3D11DeviceContext* self,
...
static inline void
_sg_d3d11_IASetIndexBuffer(ID3D11DeviceContext* self, ID3D11Buffer* pIndexBuffer, DXGI_FORMAT Format, UINT Offset)
{
#if defined(__cplusplus)
    ...
    ID3D11InputLayout**             ppInputLayout)
{
    ...
    return self->CreateInputLayout(
        ...
        ppInputLayout);
#else
    return self->lpVtbl->CreateInputLayout(
        self,
        pInputElementDescs,
        NumElements,
        pShaderBytecodeWithInputSignature,
...
_sg_d3d11_CreateBlendState(ID3D11Device* self, const D3D11_BLEND_DESC* pBlendStateDesc, ID3D11BlendState** ppBlendState)
{
#if defined(__cplusplus)
...
static inline void
_sg_d3d11_RSSetViewports(ID3D11DeviceContext* self, UINT NumViewports, const D3D11_VIEWPORT* pViewports)
{
#if defined(__cplusplus)
...
static inline void _sg_d3d11_ResolveSubresource(
    ID3D11DeviceContext* self,
...
static inline void _sg_d3d11_UpdateSubresource(
    ID3D11DeviceContext* self,
    ID3D11Resource*      pDstResource,
    UINT                 DstSubresource,
...
static inline void
_sg_d3d11_DrawIndexed(ID3D11DeviceContext* self, UINT IndexCount, UINT StartIndexLocation, INT BaseVertexLocation)
{
#if defined(__cplusplus)
...
static inline void _sg_d3d11_DrawIndexedInstanced(
    ID3D11DeviceContext* self,
...
#else
    self->lpVtbl->DrawIndexedInstanced(
        self,
        IndexCountPerInstance,
        InstanceCount,
        StartIndexLocation,
...
static inline void _sg_d3d11_DrawInstanced(
    ID3D11DeviceContext* self,
...
#else
    self->lpVtbl
        ->DrawInstanced(self, VertexCountPerInstance, InstanceCount, StartVertexLocation, StartInstanceLocation);
#endif
}

static inline HRESULT _sg_d3d11_Map(
    ID3D11DeviceContext*      self,
    ID3D11Resource*           pResource,
    UINT                      Subresource,
    D3D11_MAP                 MapType,
...
_sg_d3d11_filter(sg_filter min_f, sg_filter mag_f, sg_filter mipmap_f, bool comparison, uint32_t max_anisotropy)
{
            ...
            for (int mip_index = 0; mip_index < img->cmn.num_mipmaps; mip_index++, subres_index++)
            {
                ...
                if (img->cmn.type == SG_IMAGETYPE_3D)
                {
                    ...
                    subres_data->SysMemSlicePitch =
                        (UINT)_sg_surface_pitch(img->cmn.pixel_format, mip_width, mip_height, 1);
                }
                else
                {
...
_SOKOL_PRIVATE sg_resource_state _sg_d3d11_create_sampler(_sg_sampler_t* smp, const sg_sampler_desc* desc)
{
    ...
    else
    {
        ...
        d3d11_smp_desc.Filter = _sg_d3d11_filter(
            ...
            desc->mag_filter,
            desc->mipmap_filter,
            desc->compare != SG_COMPAREFUNC_NEVER,
            desc->max_anisotropy);
...
_SOKOL_PRIVATE sg_resource_state
_sg_d3d11_create_pipeline(_sg_pipeline_t* pip, _sg_shader_t* shd, const sg_pipeline_desc* desc)
{
    SOKOL_ASSERT(pip && shd && desc);
    SOKOL_ASSERT(desc->shader.id == shd->slot.id);
...
_SOKOL_PRIVATE sg_resource_state _sg_d3d11_create_attachments(
    _sg_attachments_t*         atts,
    _sg_image_t**              color_images,
    _sg_image_t**              resolve_images,
    _sg_image_t*               ds_img,
...
_SOKOL_PRIVATE void _sg_d3d11_begin_pass(const sg_pass* pass)
{
    ...
    if ((0 != ds_flags) && dsv)
    {
        _sg_d3d11_ClearDepthStencilView(
            _sg.d3d11.ctx,
            dsv,
            ds_flags,
...
_SOKOL_PRIVATE void _sg_d3d11_apply_pipeline(_sg_pipeline_t* pip)
{
    ...
    _sg_d3d11_VSSetConstantBuffers(
        _sg.d3d11.ctx,
        0,
        SG_MAX_SHADERSTAGE_UBS,
        pip->shader->d3d11.stage[SG_SHADERSTAGE_VS].cbufs);
    _sg_d3d11_PSSetShader(_sg.d3d11.ctx, pip->shader->d3d11.fs, NULL, 0);
    _sg_d3d11_PSSetConstantBuffers(
        _sg.d3d11.ctx,
        0,
        SG_MAX_SHADERSTAGE_UBS,
        pip->shader->d3d11.stage[SG_SHADERSTAGE_FS].cbufs);
...
_SOKOL_PRIVATE void _sg_d3d11_apply_uniforms(sg_shader_stage stage_index, int ub_index, const sg_range* data)
{
    ...
    SOKOL_ASSERT(
        ...
        _sg.d3d11.cur_pipeline->shader->slot.id == _sg.d3d11.cur_pipeline->cmn.shader_id.id);
...
_SOKOL_PRIVATE void _sg_d3d11_draw(int base_element, int num_elements, int num_instances)
{
    ...
    if (_sg.d3d11.use_indexed_draw)
    {
        if (use_instanced_draw)
        {
            _sg_d3d11_DrawIndexedInstanced(
                _sg.d3d11.ctx,
                (UINT)num_elements,
                (UINT)num_instances,
                (UINT)base_element,
...
_SOKOL_PRIVATE void _sg_d3d11_update_buffer(_sg_buffer_t* buf, const sg_range* data)
{
    ...
    HRESULT                  hr =
        _sg_d3d11_Map(_sg.d3d11.ctx, (ID3D11Resource*)buf->d3d11.buf, 0, D3D11_MAP_WRITE_DISCARD, 0, &d3d11_msr);
...
_SOKOL_PRIVATE void _sg_mtl_init_pool(const sg_desc* desc)
{
    _sg.mtl.idpool.num_slots =
        2 * (2 * desc->buffer_pool_size + 4 * desc->image_pool_size + 1 * desc->sampler_pool_size +
             4 * desc->shader_pool_size + 2 * desc->pipeline_pool_size + desc->attachments_pool_size + 128);
    ...
    // a circular queue which holds release items (frame index when a resource is to be released, and the resource's
    // pool index
    _sg.mtl.idpool.release_queue_front = 0;
    _sg.mtl.idpool.release_queue_back  = 0;
    _sg.mtl.idpool.release_queue =
        (_sg_mtl_release_item_t*)_sg_malloc_clear((size_t)_sg.mtl.idpool.num_slots * sizeof(_sg_mtl_release_item_t));
    for (int i = 0; i < _sg.mtl.idpool.num_slots; i++)
    {
...
_SOKOL_PRIVATE id _sg_mtl_id(int slot_index) { return _sg.mtl.idpool.pool[(NSUInteger)slot_index]; }

_SOKOL_PRIVATE void _sg_mtl_clear_state_cache(void) { _sg_clear(&_sg.mtl.state_cache, sizeof(_sg.mtl.state_cache)); }
...
_SOKOL_PRIVATE void _sg_mtl_init_caps(void)
{
    ...
    if (@available(macOS 12.0, iOS 14.0, *))
    {
        _sg.features.image_clamp_to_border =
            [_sg.mtl.device supportsFamily:MTLGPUFamilyApple7] || [_sg.mtl.device supportsFamily:MTLGPUFamilyMac2];
...
_SOKOL_PRIVATE void _sg_mtl_setup_backend(const sg_desc* desc)
{
    ...
    {
        _sg.mtl.uniform_buffers[i] =
            [_sg.mtl.device newBufferWithLength:(NSUInteger)_sg.mtl.ub_size
...
_SOKOL_PRIVATE void _sg_mtl_bind_uniform_buffers(void)
{
    ...
    {
        [_sg.mtl.cmd_encoder setVertexBuffer:_sg.mtl.uniform_buffers[_sg.mtl.cur_frame_rotate_index]
                                      offset:0
                                     atIndex:(NSUInteger)slot];
        [_sg.mtl.cmd_encoder setFragmentBuffer:_sg.mtl.uniform_buffers[_sg.mtl.cur_frame_rotate_index]
                                        offset:0
                                       atIndex:(NSUInteger)slot];
    }
}
...
_SOKOL_PRIVATE void
_sg_mtl_copy_image_data(const _sg_image_t* img, __unsafe_unretained id<MTLTexture> mtl_tex, const sg_image_data* data)
{
    const int num_faces  = (img->cmn.type == SG_IMAGETYPE_CUBE) ? 6 : 1;
    const int num_slices = (img->cmn.type == SG_IMAGETYPE_ARRAY) ? img->cmn.num_slices : 1;
    for (int face_index = 0; face_index < num_faces; face_index++)
        ...
        for (int mip_index = 0; mip_index < img->cmn.num_mipmaps; mip_index++)
        {
            ...
            */
            ...
            if (img->cmn.type == SG_IMAGETYPE_3D)
            {
                ...
                // FIXME: apparently the minimal bytes_per_image size for 3D texture is 4 KByte... somehow need to
                // handle this
            }
            else
            {
...
_SOKOL_PRIVATE id<MTLLibrary> _sg_mtl_compile_library(const char* src)
{
    NSError*       err = NULL;
    id<MTLLibrary> lib =
        [_sg.mtl.device newLibraryWithSource:[NSString stringWithUTF8String:src] options:nil error:&err];
    if (err)
    {
...
_SOKOL_PRIVATE sg_resource_state
_sg_mtl_create_pipeline(_sg_pipeline_t* pip, _sg_shader_t* shd, const sg_pipeline_desc* desc)
{
    SOKOL_ASSERT(pip && shd && desc);
    SOKOL_ASSERT(desc->shader.id == shd->slot.id);
...
_SOKOL_PRIVATE sg_resource_state _sg_mtl_create_attachments(
    _sg_attachments_t*         atts,
    _sg_image_t**              color_images,
    _sg_image_t**              resolve_images,
    _sg_image_t*               ds_img,
    ...
    if (atts)
    {
        ...
        for (NSUInteger i = 0; i < (NSUInteger)atts->cmn.num_colors; i++)
        {
            ...
            pass_desc.colorAttachments[i].storeAction =
                _sg_mtl_store_action(action->colors[i].store_action, resolve_att_img != 0);
            ...
            if (resolve_att_img)
            {
                ...
                pass_desc.colorAttachments[i].resolveTexture =
                    _sg_mtl_id(resolve_att_img->mtl.tex[resolve_att_img->cmn.active_slot]);
...
_SOKOL_PRIVATE void _sg_mtl_apply_pipeline(_sg_pipeline_t* pip)
{
    ...
    if (_sg.mtl.state_cache.cur_pipeline_id.id != pip->slot.id)
    {
        ...
        [_sg.mtl.cmd_encoder setDepthBias:pip->cmn.depth.bias
...
_SOKOL_PRIVATE bool _sg_mtl_apply_bindings(_sg_bindings_t* bnd)
{
    ...
    for (NSUInteger slot = 0; slot < (NSUInteger)bnd->num_vs_sbufs; slot++)
    {
        ...
        if (_sg.mtl.state_cache.cur_vs_storagebuffer_ids[slot].id != sbuf->slot.id)
        {
            ...
            [_sg.mtl.cmd_encoder setVertexBuffer:_sg_mtl_id(sbuf->mtl.buf[sbuf->cmn.active_slot])
                                          offset:0
                                         atIndex:mtl_slot];
            _sg_stats_add(metal.bindings.num_set_vertex_buffer, 1);
        }
    ...
    for (NSUInteger slot = 0; slot < (NSUInteger)bnd->num_fs_sbufs; slot++)
    {
        ...
        if (_sg.mtl.state_cache.cur_fs_storagebuffer_ids[slot].id != sbuf->slot.id)
        {
            ...
            SOKOL_ASSERT(sbuf->mtl.buf[sbuf->cmn.active_slot] != _SG_MTL_INVALID_SLOT_INDEX);
            const NSUInteger mtl_slot = SG_MAX_SHADERSTAGE_UBS + slot;
            [_sg.mtl.cmd_encoder setFragmentBuffer:_sg_mtl_id(sbuf->mtl.buf[sbuf->cmn.active_slot])
                                            offset:0
                                           atIndex:mtl_slot];
...
_SOKOL_PRIVATE void _sg_mtl_apply_uniforms(sg_shader_stage stage_index, int ub_index, const sg_range* data)
{
    ...
    SOKOL_ASSERT(
        _sg.mtl.state_cache.cur_pipeline->shader->slot.id == _sg.mtl.state_cache.cur_pipeline->cmn.shader_id.id);
    SOKOL_ASSERT(ub_index < _sg.mtl.state_cache.cur_pipeline->shader->cmn.stage[stage_index].num_uniform_blocks);
    SOKOL_ASSERT(
        data->size == _sg.mtl.state_cache.cur_pipeline->shader->cmn.stage[stage_index].uniform_blocks[ub_index].size);
...
_SOKOL_PRIVATE void _sg_wgpu_init_caps(void)
{
    ...
    _sg.limits.max_image_size_cube =
        (int)l->maxTextureDimension2D; // not a bug, see: https://github.com/gpuweb/gpuweb/issues/1327
...
_SOKOL_PRIVATE void _sg_wgpu_init_bindgroups_cache_key(_sg_wgpu_bindgroups_cache_key_t* key, const _sg_bindings_t* bnd)
{
    ...
    SOKOL_ASSERT(
        item_idx == (1 + bnd->num_vs_imgs + bnd->num_vs_smps + bnd->num_vs_sbufs + bnd->num_fs_imgs + bnd->num_fs_smps +
                     bnd->num_fs_sbufs));
    key->hash = _sg_wgpu_hash(&key->items, (int)sizeof(key->items), 0x1234567887654321);
}

_SOKOL_PRIVATE bool
_sg_wgpu_compare_bindgroups_cache_key(_sg_wgpu_bindgroups_cache_key_t* k0, _sg_wgpu_bindgroups_cache_key_t* k1)
{
    SOKOL_ASSERT(k0 && k1);
    if (k0->hash != k1->hash)
    {
...
_SOKOL_PRIVATE bool _sg_wgpu_bindings_cache_ib_dirty(const _sg_buffer_t* ib, int offset)
{
    if (ib)
    {
        return (_sg.wgpu.bindings_cache.ib.buffer.id != ib->slot.id) || (_sg.wgpu.bindings_cache.ib.offset != offset);
    }
    else
    {
...
_SOKOL_PRIVATE void _sg_wgpu_set_bindgroup(_sg_wgpu_bindgroup_t* bg)
{
    if (_sg_wgpu_bindings_cache_bg_dirty(bg))
    {
        ...
        if (bg)
        {
            ...
            wgpuRenderPassEncoderSetBindGroup(
                _sg.wgpu.pass_enc,
                _SG_WGPU_IMAGE_SAMPLER_BINDGROUP_INDEX,
                bg->bindgroup,
                0,
                0);
        }
...
{
    if ((bnd->num_vs_imgs + bnd->num_vs_smps + bnd->num_vs_sbufs + bnd->num_fs_imgs + bnd->num_fs_smps +
         bnd->num_fs_sbufs) > 0)
    {
...
_SOKOL_PRIVATE void _sg_wgpu_reset_state_cache(void) { _sg_wgpu_bindings_cache_clear(); }
...
_SOKOL_PRIVATE sg_resource_state _sg_wgpu_create_shader(_sg_shader_t* shd, const sg_shader_desc* desc)
{
    ...
    for (int stage_index = 0; stage_index < SG_NUM_SHADER_STAGES; stage_index++)
    {
        ...
        for (int sbuf_index = 0; sbuf_index < num_sbufs; sbuf_index++)
        {
            ...
            wgpu_bgl_entry->visibility = _sg_wgpu_shader_stage((sg_shader_stage)stage_index);
            wgpu_bgl_entry->buffer.type =
                sbuf_desc->readonly ? WGPUBufferBindingType_ReadOnlyStorage : WGPUBufferBindingType_Storage;
        }
    }
...
_SOKOL_PRIVATE sg_resource_state
_sg_wgpu_create_pipeline(_sg_pipeline_t* pip, _sg_shader_t* shd, const sg_pipeline_desc* desc)
{
    SOKOL_ASSERT(pip && shd && desc);
    SOKOL_ASSERT(desc->shader.id == shd->slot.id);
...
_SOKOL_PRIVATE sg_resource_state _sg_wgpu_create_attachments(
    _sg_attachments_t*         atts,
    _sg_image_t**              color_images,
    _sg_image_t**              resolve_images,
    _sg_image_t*               ds_img,
...
static inline sg_resource_state
...
// >>validation
#if defined(SOKOL_DEBUG)
_SOKOL_PRIVATE void _sg_validate_begin(void) { _sg.validate_error = SG_LOGITEM_OK; }
...
_SOKOL_PRIVATE void _sg_validate_image_data(
...
_SOKOL_PRIVATE bool _sg_validate_image_desc(const sg_image_desc* desc)
{
    ...
    const bool injected = (0 != desc->gl_textures[0]) || (0 != desc->mtl_textures[0]) || (0 != desc->d3d11_texture) ||
                          (0 != desc->wgpu_texture);
...
_SOKOL_PRIVATE bool _sg_validate_shader_desc(const sg_shader_desc* desc)
{
    ...
    for (int stage_index = 0; stage_index < SG_NUM_SHADER_STAGES; stage_index++)
    {
                ...
                for (int u_index = 0; u_index < SG_MAX_UB_MEMBERS; u_index++)
                {
                    ...
                    if (u_desc->type != SG_UNIFORMTYPE_INVALID)
                    {
                        ...
                        if (ub_desc->layout == SG_UNIFORMLAYOUT_STD140)
                        {
                            ...
                            {
                                _SG_VALIDATE(
                                    (u_desc->type == SG_UNIFORMTYPE_FLOAT4) || (u_desc->type == SG_UNIFORMTYPE_INT4) ||
        ...
        for (int img_smp_index = 0; img_smp_index < SG_MAX_SHADERSTAGE_IMAGESAMPLERPAIRS; img_smp_index++)
        {
            ...
            if (img_smp_desc->used)
            {
                ...
                const bool img_slot_in_range =
                    (img_smp_desc->image_slot >= 0) && (img_smp_desc->image_slot < SG_MAX_SHADERSTAGE_IMAGES);
                const bool smp_slot_in_range =
                    (img_smp_desc->sampler_slot >= 0) && (img_smp_desc->sampler_slot < SG_MAX_SHADERSTAGE_SAMPLERS);
                _SG_VALIDATE(
                    img_slot_in_range && (img_smp_desc->image_slot < num_images),
                    VALIDATE_SHADERDESC_IMAGE_SAMPLER_PAIR_IMAGE_SLOT_OUT_OF_RANGE);
                _SG_VALIDATE(
                    smp_slot_in_range && (img_smp_desc->sampler_slot < num_samplers),
                    VALIDATE_SHADERDESC_IMAGE_SAMPLER_PAIR_IMAGE_SLOT_OUT_OF_RANGE);
#if defined(_SOKOL_ANY_GL)
                ...
                if (img_slot_in_range && smp_slot_in_range)
                {
                    ...
                    {
                        _SG_VALIDATE(
                            needs_nonfiltering && (smp_desc->sampler_type == SG_SAMPLERTYPE_NONFILTERING),
                            VALIDATE_SHADERDESC_NONFILTERING_SAMPLER_REQUIRED);
                    }
                    if (needs_comparison)
                    {
                        _SG_VALIDATE(
                            needs_comparison && (smp_desc->sampler_type == SG_SAMPLERTYPE_COMPARISON),
        ...
        _SG_VALIDATE(
            expected_img_slot_mask == actual_img_slot_mask,
            VALIDATE_SHADERDESC_IMAGE_NOT_REFERENCED_BY_IMAGE_SAMPLER_PAIRS);
        _SG_VALIDATE(
            expected_smp_slot_mask == actual_smp_slot_mask,
...
_SOKOL_PRIVATE bool _sg_validate_attachments_desc(const sg_attachments_desc* desc)
{
        ...
        if (0 != img)
        {
            ...
            if (res_att->image.id != SG_INVALID_ID)
            {
                ...
                if (res_img != 0)
                {
                    ...
                    _SG_VALIDATE(
                        res_att->mip_level < res_img->cmn.num_mipmaps,
                    ...
                    _SG_VALIDATE(
                        img->cmn.pixel_format == res_img->cmn.pixel_format,
                        VALIDATE_ATTACHMENTSDESC_RESOLVE_IMAGE_FORMAT);
                    _SG_VALIDATE(
                        color_width == _sg_miplevel_dim(res_img->cmn.width, res_att->mip_level),
                        VALIDATE_ATTACHMENTSDESC_RESOLVE_IMAGE_SIZES);
                    _SG_VALIDATE(
                        color_height == _sg_miplevel_dim(res_img->cmn.height, res_att->mip_level),
                        VALIDATE_ATTACHMENTSDESC_RESOLVE_IMAGE_SIZES);
                }
            }
        }
    ...
    if (desc->depth_stencil.image.id != SG_INVALID_ID)
    {
        ...
        if (img)
        {
            ...
            _SG_VALIDATE(
                (color_width == -1) || (color_width == _sg_miplevel_dim(img->cmn.width, att->mip_level)),
                VALIDATE_ATTACHMENTSDESC_DEPTH_IMAGE_SIZES);
            _SG_VALIDATE(
                (color_height == -1) || (color_height == _sg_miplevel_dim(img->cmn.height, att->mip_level)),
                VALIDATE_ATTACHMENTSDESC_DEPTH_IMAGE_SIZES);
            _SG_VALIDATE(
                (color_sample_count == -1) || (color_sample_count == img->cmn.sample_count),
...
_SOKOL_PRIVATE bool _sg_validate_begin_pass(const sg_pass* pass)
{
    ...
    if (pass->attachments.id == SG_INVALID_ID)
    {
        ...
        _SG_VALIDATE(
            pass->swapchain.color_format > SG_PIXELFORMAT_NONE,
...
// NOTE: the GL framebuffer handle may actually be 0
        ...
        {
            _SG_VALIDATE(
                pass->swapchain.wgpu.resolve_view == 0,
    ...
    else
    {
        ...
        if (atts)
        {
            ...
            for (int i = 0; i < SG_MAX_COLOR_ATTACHMENTS; i++)
            {
                ...
                {
                    _SG_VALIDATE(
                        color_img->slot.state == SG_RESOURCESTATE_VALID,
                        VALIDATE_BEGINPASS_COLOR_ATTACHMENT_IMAGE);
                    _SG_VALIDATE(
                        color_img->slot.id == color_att->image_id.id,
                        VALIDATE_BEGINPASS_COLOR_ATTACHMENT_IMAGE);
                }
                ...
                {
                    _SG_VALIDATE(
                        resolve_img->slot.state == SG_RESOURCESTATE_VALID,
                        VALIDATE_BEGINPASS_RESOLVE_ATTACHMENT_IMAGE);
                    _SG_VALIDATE(
                        resolve_img->slot.id == resolve_att->image_id.id,
                        VALIDATE_BEGINPASS_RESOLVE_ATTACHMENT_IMAGE);
                }
            }
            ...
            if (ds_img)
            {
                ...
                _SG_VALIDATE(
                    ds_img->slot.state == SG_RESOURCESTATE_VALID,
        ...
        _SG_VALIDATE(
            pass->swapchain.color_format == _SG_PIXELFORMAT_DEFAULT,
            VALIDATE_BEGINPASS_SWAPCHAIN_EXPECT_COLORFORMAT_NOTSET);
        _SG_VALIDATE(
            pass->swapchain.depth_format == _SG_PIXELFORMAT_DEFAULT,
...
#elif defined(SOKOL_D3D11)
        _SG_VALIDATE(
            pass->swapchain.d3d11.render_view == 0,
...
_SOKOL_PRIVATE bool _sg_validate_apply_bindings(const sg_bindings* bindings)
{
        ...
        if (stage->samplers[i].sampler_type != _SG_SAMPLERTYPE_DEFAULT)
        {
            ...
            if (bindings->vs.samplers[i].id != SG_INVALID_ID)
            {
                ...
                if (smp)
                {
                    ...
                    {
                        _SG_VALIDATE(
                            smp->cmn.compare != SG_COMPAREFUNC_NEVER,
    ...
    for (int i = 0; i < SG_MAX_SHADERSTAGE_STORAGEBUFFERS; i++)
    {
        ...
        {
            _SG_VALIDATE(
                bindings->vs.storage_buffers[i].id != SG_INVALID_ID,
            ...
            if (bindings->vs.storage_buffers[i].id != SG_INVALID_ID)
            {
                ...
                {
                    _SG_VALIDATE(
                        sbuf->cmn.type == SG_BUFFERTYPE_STORAGEBUFFER,
    ...
    for (int i = 0; i < SG_MAX_SHADERSTAGE_STORAGEBUFFERS; i++)
    {
        ...
        {
            _SG_VALIDATE(
                bindings->fs.storage_buffers[i].id != SG_INVALID_ID,
...
{
    SOKOL_ASSERT(
        atts && ((atts->slot.state == SG_RESOURCESTATE_VALID) || (atts->slot.state == SG_RESOURCESTATE_FAILED)));
...
#endif
    res.environment.defaults.depth_format =
        _sg_def(res.environment.defaults.depth_format, SG_PIXELFORMAT_DEPTH_STENCIL);
...
SOKOL_API_IMPL bool sg_frame_stats_enabled(void) { return _sg.stats_enabled; }

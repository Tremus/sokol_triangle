...
#include <windowsx.h>
#if ! defined(                                                                                                         \
    SOKOL_NO_ENTRY) // if SOKOL_NO_ENTRY is defined, it's the applications' responsibility to use the right subsystem
...
_SOKOL_PRIVATE bool _sapp_ring_full(_sapp_ring_t* ring) { return _sapp_ring_idx(ring->head + 1) == ring->tail; }

_SOKOL_PRIVATE bool _sapp_ring_empty(_sapp_ring_t* ring) { return ring->head == ring->tail; }
...
_SOKOL_PRIVATE double _sapp_timestamp_now(_sapp_timestamp_t* ts)
{
    ...
    const uint64_t now =
        (uint64_t)_sapp_int64_muldiv((int64_t)traw, (int64_t)ts->mach.timebase.numer, (int64_t)ts->mach.timebase.denom);
    ...
    const uint64_t now =
        (uint64_t)_sapp_int64_muldiv(qpc.QuadPart - ts->win.start.QuadPart, 1000000000, ts->win.freq.QuadPart);
    return (double)now / 1000000000.0;
#else
    struct timespec tspec;
    clock_gettime(_SAPP_CLOCK_MONOTONIC, &tspec);
...
_SOKOL_PRIVATE void _sapp_timing_discontinuity(_sapp_timing_t* t) { t->last = 0.0; }
...
_SOKOL_PRIVATE void _sapp_init_state(const sapp_desc* desc)
{
    ...
    _sapp_strcpy(
        ...
        &_sapp.html5_canvas_selector[1],
        sizeof(_sapp.html5_canvas_selector) - 1);
...
_SOKOL_PRIVATE void _sapp_macos_init_cursors(void)
{
    ...
    _sapp.macos.cursors[SAPP_MOUSECURSOR_RESIZE_EW] =
        [NSCursor respondsToSelector:@selector(_windowResizeEastWestCursor)] ? [NSCursor _windowResizeEastWestCursor]
...
_SOKOL_PRIVATE void _sapp_macos_update_dimensions(void)
{
    ...
    _sapp.macos.view.layer.contentsScale = _sapp.dpi_scale; // NOTE: needed because we set layerContentsPlacement to a
                                                            // non-scaling value in windowWillStartLiveResize.
...
_SOKOL_PRIVATE void _sapp_macos_set_icon(const sapp_icon_desc* icon_desc, int num_images)
{
    ...
    CFDataRef       cf_data =
        CFDataCreate(kCFAllocatorDefault, (const UInt8*)img_desc->pixels.ptr, (CFIndex)img_desc->pixels.size);
...
- (void)applicationDidFinishLaunching:(NSNotification*)aNotification
{
    ...
    _sapp.macos.window                         = [[_sapp_macos_window alloc] initWithContentRect:window_rect
                                                               styleMask:style
    ...
    _sapp.macos.view = [[_sapp_macos_view alloc] initWithFrame:window_rect pixelFormat:glpixelformat_obj];
...
- (BOOL)performDragOperation:(id<NSDraggingInfo>)sender
{
    ...
    if ([pboard.types containsObject:NSPasteboardTypeFileURL])
    {
        ...
        _sapp.drop.num_files = ((int)pboard.pasteboardItems.count > _sapp.drop.max_files)
        ...
        {
            NSURL* fileUrl =
                [NSURL fileURLWithPath:[pboard.pasteboardItems[(NSUInteger)i] stringForType:NSPasteboardTypeFileURL]];
            if (! _sapp_strcpy(
                    fileUrl.standardizedURL.path.UTF8String,
...
_SOKOL_PRIVATE void _sapp_macos_poll_input_events(void)
{
    ...
    @autoreleasepool {
        for (;;) {
            ...
            NSEvent* event = [NSApp nextEventMatchingMask:mask untilDate:nil inMode:NSEventTrackingRunLoopMode
    dequeue:YES]; if (event == nil) { break;
            }
...
- (void)flagsChanged:(NSEvent*)event
{
    ...
    {
        _sapp_macos_key_event(
            down ? SAPP_EVENTTYPE_KEY_DOWN : SAPP_EVENTTYPE_KEY_UP,
            key_code,
            false,
            _sapp_macos_mods(event));
    }
...
- (BOOL)application:(UIApplication*)application didFinishLaunchingWithOptions:(NSDictionary*)launchOptions
{
    ...
    */
    ...
    GLKViewDrawableMultisample msaa =
        _sapp.sample_count > 1 ? GLKViewDrawableMultisample4X : GLKViewDrawableMultisampleNone;
...
EMSCRIPTEN_KEEPALIVE int _sapp_html5_get_ask_leave_site(void) { return _sapp.html5_ask_leave_site ? 1 : 0; }
...
EM_JS(void, sapp_js_remove_clipboard_listener, (void), { window.removeEventListener('paste', Module.sokol_paste); });
...
_SOKOL_PRIVATE void _sapp_emsc_set_clipboard_string(const char* str) { sapp_js_write_clipboard(str); }
...
EM_JS(
    ...
    (int index, _sapp_html5_fetch_callback callback, void* buf_ptr, uint32_t buf_size, void* user_data),
    {
...
_sapp_emsc_pointerlockchange_cb(int emsc_type, const EmscriptenPointerlockChangeEvent* emsc_event, void* user_data)
{
...
_SOKOL_PRIVATE bool _sapp_emsc_is_char_key(sapp_keycode key_code) { return key_code < SAPP_KEYCODE_WORLD_1; }
    ...
    if (_sapp_events_enabled())
    {
        ...
        if (type != SAPP_EVENTTYPE_INVALID)
        {
            ...
            else
            {
                ...
                if ((type == SAPP_EVENTTYPE_KEY_DOWN) && (_sapp.event.key_code != SAPP_KEYCODE_LEFT_SUPER) &&
                    (_sapp.event.key_code != SAPP_KEYCODE_RIGHT_SUPER) && (_sapp.event.modifiers & SAPP_MODIFIER_SUPER))
                {
...
_SOKOL_PRIVATE void
_sapp_emsc_wgpu_request_device_cb(WGPURequestDeviceStatus status, WGPUDevice device, const char* msg, void* userdata)
{
    _SOKOL_UNUSED(msg);
...
_SOKOL_PRIVATE bool _sapp_gl_select_fbconfig(
...
_sapp_gl_choose_fbconfig(const _sapp_gl_fbconfig* desired, const _sapp_gl_fbconfig* alternatives, int count)
{
...
static inline HRESULT _sapp_dxgi_ResizeBuffers(
    IDXGISwapChain* self,
    UINT            BufferCount,
    UINT            Width,
    UINT            Height,
...
_SOKOL_PRIVATE void _sapp_d3d11_create_device_and_swapchain(void)
{
    ...
    if (SUCCEEDED(hr) && _sapp.d3d11.dxgi_device)
    {
        ...
        if (SUCCEEDED(hr) && dxgi_adapter)
        {
            ...
            hr                         = _sapp_dxgi_GetParent(
                (IDXGIObject*)dxgi_adapter,
...
_SOKOL_PRIVATE void _sapp_d3d11_create_default_render_target(void)
{
    ...
    hr =
        _sapp_d3d11_CreateRenderTargetView(_sapp.d3d11.device, (ID3D11Resource*)_sapp.d3d11.rt, NULL, &_sapp.d3d11.rtv);
    ...
    if (_sapp.sample_count > 1)
    {
        ...
        hr = _sapp_d3d11_CreateRenderTargetView(
            _sapp.d3d11.device,
            (ID3D11Resource*)_sapp.d3d11.msaa_rt,
            NULL,
    ...
    hr =
        _sapp_d3d11_CreateDepthStencilView(_sapp.d3d11.device, (ID3D11Resource*)_sapp.d3d11.ds, NULL, &_sapp.d3d11.dsv);
...
_SOKOL_PRIVATE void _sapp_d3d11_resize_default_render_target(void)
{
    if (_sapp.d3d11.swap_chain)
    {
        ...
        _sapp_dxgi_ResizeBuffers(
            _sapp.d3d11.swap_chain,
            _sapp.d3d11.swap_chain_desc.BufferCount,
            (UINT)_sapp.framebuffer_width,
            (UINT)_sapp.framebuffer_height,
...
_SOKOL_PRIVATE void _sapp_wgl_load_extensions(void)
{
    ...
    _sapp.wgl.GetExtensionsStringEXT =
        (PFNWGLGETEXTENSIONSSTRINGEXTPROC)(void*)_sapp.wgl.GetProcAddress("wglGetExtensionsStringEXT");
    _sapp.wgl.GetExtensionsStringARB =
        (PFNWGLGETEXTENSIONSSTRINGARBPROC)(void*)_sapp.wgl.GetProcAddress("wglGetExtensionsStringARB");
    _sapp.wgl.CreateContextAttribsARB =
        (PFNWGLCREATECONTEXTATTRIBSARBPROC)(void*)_sapp.wgl.GetProcAddress("wglCreateContextAttribsARB");
    ...
    _sapp.wgl.GetPixelFormatAttribivARB =
        (PFNWGLGETPIXELFORMATATTRIBIVARBPROC)(void*)_sapp.wgl.GetProcAddress("wglGetPixelFormatAttribivARB");
...
#define _sapp_wgl_num_query_tags (12)
    ...
    //  If in future querying with multiple extensions, will have to shuffle index values to have active extensions on
    //  the end.
    ...
    for (int i = 0; i < native_count; i++)
    {
        ...
        if (query_results[result_support_opengl_index] == 0 || query_results[result_draw_to_window_index] == 0 ||
        ...
        u.samples = query_results[result_samples_index]; // NOTE: If arb_multisample is not supported  - just takes the
                                                         // default 0
...
_SOKOL_PRIVATE void _sapp_win32_key_event(sapp_event_type type, int vk, bool repeat)
{
    if (_sapp_events_enabled() && (vk < SAPP_MAX_KEYCODES))
    {
        ...
        if (_sapp.clipboard.enabled && (type == SAPP_EVENTTYPE_KEY_DOWN) &&
            (_sapp.event.modifiers == SAPP_MODIFIER_CTRL) && (_sapp.event.key_code == SAPP_KEYCODE_V))
        {
            _sapp_init_event(SAPP_EVENTTYPE_CLIPBOARD_PASTED);
            _sapp_call_event(&_sapp.event);
        }
        ...
        if (SUCCEEDED(hr))
        {
            if (dxgi_stats.SyncRefreshCount != _sapp.d3d11.sync_refresh_count)
            {
                ...
                const uint64_t now             = (uint64_t)_sapp_int64_muldiv(
                    ...
                    1000000000,
                    _sapp.timing.timestamp.win.freq.QuadPart);
...
_SOKOL_PRIVATE void _sapp_win32_create_window(void)
{
    ...
    */
    ...
    _sapp.win32.hwnd             = CreateWindowExW(
        ...
        use_default_height ? CW_USEDEFAULT
                                       : win_height, // nHeight (NOTE: if width is CW_USEDEFAULT, height is actually ignored)
        NULL,                            // hWndParent
        NULL,                            // hMenu
...
_SOKOL_PRIVATE void _sapp_win32_init_dpi(void)
{
    ...
    if (user32)
    {
        ...
        fn_setprocessdpiawarenesscontext =
            (SETPROCESSDPIAWARENESSCONTEXT_T)(void*)GetProcAddress(user32, "SetProcessDpiAwarenessContext");
    }
...
_SOKOL_PRIVATE void _sapp_win32_set_icon(const sapp_icon_desc* icon_desc, int num_images)
{
    ...
    int big_img_index =
        _sapp_image_bestmatch(icon_desc->images, num_images, GetSystemMetrics(SM_CXICON), GetSystemMetrics(SM_CYICON));
    int sml_img_index = _sapp_image_bestmatch(
        icon_desc->images,
        num_images,
...
_SOKOL_PRIVATE bool _sapp_android_init_egl(void)
{
    ...
    for (int i = 0; i < cfg_count; ++i)
    {
        ...
        if (eglGetConfigAttrib(display, c, EGL_RED_SIZE, &r) == EGL_TRUE &&
            ...
            eglGetConfigAttrib(display, c, EGL_DEPTH_SIZE, &d) == EGL_TRUE && r == 8 && g == 8 && b == 8 &&
            (alpha_size == 0 || a == alpha_size) && d == 16)
        {
    ...
    if (win_changed || force_update)
    {
        if (! _sapp.desc.high_dpi)
        {
            ...
            EGLBoolean    egl_result =
                eglGetConfigAttrib(_sapp.android.display, _sapp.android.config, EGL_NATIVE_VISUAL_ID, &format);
...
_SOKOL_PRIVATE void _sapp_x11_init_extensions(void)
{
    ...
    if (XQueryExtension(
            _sapp.x11.display,
            "XInputExtension",
            &_sapp.x11.xi.major_opcode,
            &_sapp.x11.xi.event_base,
...
_SOKOL_PRIVATE void _sapp_glx_init(void)
{
    ...
    _sapp.glx.QueryExtensionsString =
        (PFNGLXQUERYEXTENSIONSSTRINGPROC)dlsym(_sapp.glx.libgl, "glXQueryExtensionsString");
    ...
    _sapp.glx.GetVisualFromFBConfig =
        (PFNGLXGETVISUALFROMFBCONFIGPROC)dlsym(_sapp.glx.libgl, "glXGetVisualFromFBConfig");
    if (! _sapp.glx.GetFBConfigs || ! _sapp.glx.GetFBConfigAttrib || ! _sapp.glx.GetClientString ||
        ! _sapp.glx.QueryExtension || ! _sapp.glx.QueryVersion || ! _sapp.glx.DestroyContext ||
        ! _sapp.glx.MakeCurrent || ! _sapp.glx.SwapBuffers || ! _sapp.glx.QueryExtensionsString ||
        ! _sapp.glx.CreateWindow || ! _sapp.glx.DestroyWindow || ! _sapp.glx.GetProcAddress ||
    ...
    if (_sapp_glx_extsupported("GLX_ARB_create_context", exts))
    {
        _sapp.glx.CreateContextAttribsARB =
            (PFNGLXCREATECONTEXTATTRIBSARBPROC)_sapp_glx_getprocaddr("glXCreateContextAttribsARB");
...
_SOKOL_PRIVATE void _sapp_glx_swap_buffers(void) { _sapp.glx.SwapBuffers(_sapp.x11.display, _sapp.glx.window); }
...
_SOKOL_PRIVATE void _sapp_x11_send_event(Atom type, int a, int b, int c, int d, int e)
{
    ...
    XSendEvent(_sapp.x11.display, _sapp.x11.root, False, SubstructureNotifyMask | SubstructureRedirectMask, &event);
}
...
_SOKOL_PRIVATE void _sapp_x11_set_fullscreen(bool enable)
{
    ...
    if (_sapp.x11.NET_WM_STATE && _sapp.x11.NET_WM_STATE_FULLSCREEN)
    {
        if (enable)
        {
            ...
            _sapp_x11_send_event(_sapp.x11.NET_WM_STATE, _NET_WM_STATE_ADD, _sapp.x11.NET_WM_STATE_FULLSCREEN, 0, 1, 0);
        }
        else
        {
...
_SOKOL_PRIVATE void _sapp_x11_create_standard_cursor(
...
_SOKOL_PRIVATE void _sapp_x11_set_icon(const sapp_icon_desc* icon_desc, int num_images)
{
    ...
    for (int img_index = 0; img_index < num_images; img_index++)
    {
        ...
        *dst++                          = img_desc->height;
        ...
        {
            *dst++ = ((long)(src[pixel_index * 4 + 0]) << 16) | ((long)(src[pixel_index * 4 + 1]) << 8) |
                     ((long)(src[pixel_index * 4 + 2]) << 0) | ((long)(src[pixel_index * 4 + 3]) << 24);
        }
    }
    XChangeProperty(
        _sapp.x11.display,
...
_sapp_x11_get_window_property(Window window, Atom property, Atom type, unsigned char** value)
{
    Atom          actualType;
    int           actualFormat;
    unsigned long itemCount, bytesAfter;
...
_SOKOL_PRIVATE bool _sapp_x11_parse_dropped_files_list(const char* src)
{
    ...
    */
    ...
    while (0 != (src_chr = *src++))
    {
        ...
        if (src_count <= 7)
        {
            if (((src_count == 1) && (src_chr != 'f')) || ((src_count == 2) && (src_chr != 'i')) ||
                ((src_count == 3) && (src_chr != 'l')) || ((src_count == 4) && (src_chr != 'e')) ||
                ((src_count == 5) && (src_chr != ':')) || ((src_count == 6) && (src_chr != '/')) ||
                ((src_count == 7) && (src_chr != '/')))
            {
...
_SOKOL_PRIVATE void _sapp_egl_init(void)
{
    ...
    for (int i = 0; i < config_count; ++i)
    {
        ...
        if (eglGetConfigAttrib(_sapp.egl.display, c, EGL_RED_SIZE, &r) &&
            ...
            eglGetConfigAttrib(_sapp.egl.display, c, EGL_SAMPLES, &n) && (r == 8) && (g == 8) && (b == 8) &&
            (a == alpha_size) && (d == 24) && (s == 8) && (n == sample_count))
        {
            config = c;
            break;
        }
...
SOKOL_API_IMPL double sapp_frame_duration(void) { return _sapp_timing_get_avg(&_sapp.timing); }
...
SOKOL_API_IMPL int sapp_height(void) { return (_sapp.framebuffer_height > 0) ? _sapp.framebuffer_height : 1; }
...
SOKOL_API_IMPL int sapp_depth_format(void) { return _SAPP_PIXELFORMAT_DEPTH_STENCIL; }

SOKOL_API_IMPL int sapp_sample_count(void) { return _sapp.sample_count; }

SOKOL_API_IMPL bool sapp_high_dpi(void) { return _sapp.desc.high_dpi && (_sapp.dpi_scale >= 1.5f); }
...
SOKOL_API_IMPL bool sapp_keyboard_shown(void) { return _sapp.onscreen_keyboard_shown; }
...
SOKOL_API_IMPL sapp_mouse_cursor sapp_get_mouse_cursor(void) { return _sapp.mouse.current_cursor; }
...
SOKOL_API_IMPL void sapp_html5_ask_leave_site(bool ask) { _sapp.html5_ask_leave_site = ask; }
